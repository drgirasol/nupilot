...
function wrapper () { // wrapper for injection
	...
	collectorAPS.prototype.loadCargo = function(aps)
	{
		...
		if (this.ooiPriority != "mcs" && this.ooiPriority != "cla")
		{
			...
			} else if (this.ooiPriority == "dur")
			{
				...
				baseSequence = [ { res: "mol", value: parseInt(aps.planet.molybdenum) }, { res: "tri", value: pasreInt(aps.planet.tritantium) } ];
	...
	distributorAPS.prototype.getShipStatus = function()
	{
		// return if ship is at Base (Source) or on route to Target (Sink)
	};
	...
	APS.prototype.objectInRangeOfEnemy = function(object)
	{
		...
		for (var j = 0; j < eS.length; j++)
		{
			...
			{
				console.log("...position (" + object.x + "/" + object.y + ") - (" + eS[j].x + "/" + eS[j].y + ") is in range (" + curDistToEnemy + " lj) of enemy ship!");
				return true;
			}
		}
		return false;
	...
	APS.prototype.evaluateMissionDestinations = function()
	{
		...
		for (var i = 0; i < this.potDest.length; i++)
		{
			...
			// don't use bases as destination, except your own
			if (hasBase && hasBase.planetid != this.base.id)
			{
				// exception(s)... clan distributors are allowed to get colonists from base planets
			...
			// not neccessary... the next stop has to be save, not the final destination
	...
	APS.prototype.setShipTarget = function(x, y)
	{
		...
		{
			if (this.isSaveShipTarget({ x: turnTargets[0].x, y: turnTargets[0].y }))
			{
	...
	var autopilot = {
		...
		populateFrnnMinefields: function()
		{
			...
			vgap.minefields.forEach(function(minefield) {
				if (minefield.ownerid != vgap.player.id) autopilot.frnnEnemyMinefields.push({x: minefield.x, y: minefield.y, radius: minefield.radius, owner: minefield.ownerid});
			});
		},
		...
		populateFrnnPlanets: function()
		{
			...
			vgap.planets.forEach(function(planet) {
				...
				if (planet.ownerid > 0 && planet.ownerid != vgap.player.id) autopilot.frnnEnemyPlanets.push({x: planet.x, y: planet.y});
